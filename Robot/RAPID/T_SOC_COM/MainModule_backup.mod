MODULE MainModule
    ...
    PROC ServerStart(bool Recover)
        ...
        IF (scaleSocketStatus = SOCKET_LISTENING) AND (commandSocketStatus = SOCKET_LISTENING)THEN
            ...
            SocketAccept scaleServerSocket, scaleClientSocketClientAddress:=clientIp, Time:=SOCKET_TIMEOUT;
            
            ! Validate client IP against whitelist
            IF IsIpAllowed(clientIp) THEN
                TPWrite "Scale client accepted from " + clientIp;
                
                ! Accept command client connection with timeout
                SocketAccept commandServerSocket, commandClientSocketClientAddress:=clientIp, Time:=SOCKET_TIMEOUT;
                
                ! Validate command client IP (should be same as scale client)
                IF IsIpAllowed(clientIp) THEN
                ...
                ELSE
                    TPWrite "Command client IP " + clientIp + " not allowed!";
                    CloseAllSockets();
                ENDIF
            ELSE
                TPWrite "Scale client IP " + clientIp + " not allowed!";
                CloseAllSockets();
            ENDIF
        ELSE
    ...
    PROC main()
        ...
        WHILE isClientConnected = TRUE DO
            TRYNEXT
                ! ----------------------------
                ! Handle Scale Server Communication
                ! ----------------------------
    ...
    FUNC bool IsIpAllowed(string ip)
        VAR bool isAllowed := FALSE;
        
        FOR i FROM 1 TO Dim(allowed_ips, 1) DO
            IF ip = allowed_ips{i} THEN
                isAllowed := TRUE;
                EXIT;
            ENDIF
        ENDFOR
